...
| `debugview <off\|normals\|detail\|steps\|albedo\|shadow\|rough\|elevzebra\|distzebra>` | replace terrain pixels with a debug channel: normals, detail height / amplitude, march steps (blue few .. red the budget), albedo, sun shadow x Lambert, roughness/rock/snow as R/G/B, and zebra stripes every 25 m of elevation / 100 m of hit distance (broken or jagged stripes = height or convergence jitter). Not persisted |
